...
if 1:
    ...
    kk = ['TC_ll', 'AM_t_1950_y', 'SLP_DJF'][:1]
    #===========================================
    for key in kk:
        print(key)
        
        odir0_0 = 'output_20220201/'+key+'/' 
        di = xr.open_dataset('input_pp/'+key+'.nc' )
        print(di.n_sample.size)
        #print(di.time)
...
for nt in range(10)[:]:
    ...
    for key in kk:
        ...
        odir0 = 'output_20220318/'+key+'/'+ '%.2d' % (nt) +'/'
        ...
        for sim in ['ssim', 'ed', 'md', 'str'][:]: 
            ...
            ofile_sbtw = odir0_0 + sim+'_sim_btw.nc' 
            if 0:
...
dt.to_csv('runtime.csv', index=None) 
